rm(list=ls())
#setwd("/home/ghepard/PDOR/simul_EBI_retreat/")
library(svglite)
library(pheatmap)
...
tree <- read.tree("RAxML_bipartitions.coreSNPs_Phylo.nwk")
...
#+geom_fruit(geom=geom_point,mapping=aes(color=cluster),size=3)


hm <- gheatmap(pp,mtab, offset = 2, width=0.2, font.size=5, colnames_position= "top",
         colnames_angle = 90, colnames_offset_y = 0, hjust = 0) +
